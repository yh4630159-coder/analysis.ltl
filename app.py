...
# ================= 3. 界面逻辑 =================
st.set_page_config(page_title="海外仓库存分析 V2.7 Fix", page_icon="🏭", layout="wide")
st.title("🏭 海外仓分仓库存分析 (V2.7 修复版)")
st.caption("✅ 已修复：解决库龄选项重复导致的报错问题")
...
if not dfs:
    ...
    # 🌟 修复点 1：移除多余的拼接，直接用 AGE_LABELS 做排序依据
    ...
    # 🌟 修复点 2：这里也不要再加 ['360天+'] 了，直接用 set 去重最稳
    present_ages = target_df['Age_Range'].unique().tolist()
    # 过滤出合法的库龄段并排序
    ...
    if not sorted_ages:
        ...
        # 默认选最右边（最老库存）
        age_rng = st.radio("选择库龄段：", sorted_ages, horizontal=True, index=len(sorted_ages)-1)
